...
$(IMPORTDIR)/chmo_import.owl: $(MIRRORDIR)/chmo.owl $(IMPORTDIR)/chmo_terms.txt \
	...
	$(ROBOT) annotate --input $< --remove-annotations \
		 ...
		 extract --term-file $(IMPORTDIR)/chmo_terms.txt $(T_IMPORTSEED) \
		         ...
		         --method BOT \
		 remove $(foreach p, $(ANNOTATION_PROPERTIES), --term $(p)) \
		        --term-file $(IMPORTDIR)/chmo_terms.txt $(T_IMPORTSEED) \
		        --select complement --select annotation-properties \
		 remove --term UBERON:0001062 \
		 		--term CHEBI:23367 \
				--term OBI:0000963 \
		 		--select "self descendants" \
		 remove -T $(IMPORTDIR)/chmo_terms_to_remove.txt --select "self instances descendants" \
		 odk:normalize --base-iri http://purl.obolibrary.org/obo \
		               --subset-decls true --synonym-decls true \
		 repair --merge-axiom-annotations true \
		 $(ANNOTATE_CONVERT_FILE)
